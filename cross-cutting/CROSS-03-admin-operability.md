...
**Default thresholds (the starting point when presenting options — founder may adjust):**
- **Soft alerts** at 50% / 75% / 90% of the agreed monthly budget (matches Layer 6 rule 2).
- **Hard cap** at **2× the agreed monthly budget** — a runaway loop, not normal growth, is the only
  thing that doubles a bill mid-month. Normal growth trips the 90% alert first, giving time to raise
  the budget deliberately.
- **Per-request AI ceiling:** no single user request may spend more than **1% of the daily AI budget**
  (blocks the "one request loops a top-tier model" failure without throttling normal use).
- **Measuring cost per feature:** tag every resource (Layer 6 rule 10) and log per-AI-call cost with
  a feature label (CROSS-02); the admin cost view groups spend by those labels. If a feature's cost
  can't be attributed, that is a Gap Report item — untracked spend is unmanageable spend.

**Alert-fatigue rule:** monitoring may expose everything, but ALERTS are reserved for actionable
thresholds only (budget %, error-rate spike, hard-cap trip, security events). If the operator gets
more than a handful of alerts a week in normal operation, the thresholds are wrong — tune them, don't
train the operator to ignore them.
