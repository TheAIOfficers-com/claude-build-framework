...
## Archival (append-only docs only)
Append-only docs grow forever by design — but a 10,000-line build-log stops being readable. Rule:
- When `prompts.md`, `build-log.md`, or `decisions.md` passes **~1,500 lines**, move the older
  entries to `archive/<filename>-YYYY.md` (e.g. `archive/build-log-2026.md`), keeping roughly the
  most recent 200 lines in the live file plus one line at the top: "Older entries: see archive/".
- Archiving MOVES entries verbatim; it never edits or summarises them (append-only stays honest).
- `glossary.md` is exempt — it stays small; prune only true duplicates.
- Sessions read the live file by default and open archives only when investigating history.
