...
3. **Read the project's planning/memory docs (see Section 8) to reload live project memory** —
especially `todo.md` (what's built / what's next) and `techdebt.md` (open issues to resolve).
4. **Declare which of the 13 layers today's work touches** (a feature touching the API, database, and
auth touches Layers 2, 3, 4 — plus Layer 8 if it handles sensitive data). Then read ONLY:
   - the `layers/LAYER-XX-\*.md` docs for the declared layers (must be **ACTIVE**),
   - every **ACTIVE** `cross-cutting/CROSS-XX-\*.md` doc (these always apply),
   - any `reference-architectures/REF-XX-\*.md` doc whose capability the build uses.
   **Exceptions — read ALL 13 layer docs when:** (a) it is a brand-new project (the gate docs are
   being written), (b) a phase is being certified against the Definition of Done, or (c) the work is
   an audit/review of the whole system. When in doubt about whether a layer is touched, read it.
5. Before building a feature, restate which layers it touches and confirm each layer's aim is met.
...
6. At the end of a work session, **update `todo.md`, `techdebt.md`, and the append-only docs**; if a
...
them at the end is what keeps that memory live across sessions. Step 4 is the "read what you need"
rule: reading every layer doc every session wastes most of the AI's working memory before any code is
written. The layers still all apply — the Definition of Done checks all 13 — but a session only loads
the ones today's work actually touches.
...
* `claude-integration/` — Claude Code-native wiring (sub-agents, slash commands, hooks, settings
template). Used only when Claude Code is the builder; see Section 15 rule 7.
...
## 14\. Operating rules (how the builder AI works, whatever model is running)

🟩 **FOR THE FOUNDER** — the sections above say WHAT gets built; this section pins down HOW the AI
behaves while building it. These habits are what separate a reliable builder from a confident-sounding
one. Any model reading this framework must follow them.

🟦 **FOR CLAUDE CODE (and any builder AI) — always:**

1. **Verify, don't assume.** Never claim a file, function, or behaviour exists without reading it
   this session. Label claims as fact (read/ran it), inference, or assumption — never present an
   assumption as fact to sound confident.
2. **Root cause first.** Reproduce and diagnose before fixing. Never patch a symptom. Before fixing
   any bug, sweep the codebase for the same pattern elsewhere and fix all instances together.
3. **Prove it works.** Run or test every change and show the output. "Should work" is not done;
   "ran it, here is the output" is done. Never mark work complete without evidence.
4. **Report honestly.** Lead with the outcome. Failed tests, skipped steps, and unmet criteria are
   stated plainly — never a success report without verification. (Pairs with Section 11: no
   sycophancy, push back with specifics.)
5. **Smallest change that solves it.** Don't refactor, rename, or "improve" code the task didn't
   touch. Mid-task ideas go to `productdebt.md` — never silent scope growth.
6. **Finish the turn.** No response ends with an unfulfilled promise ("I'll do X next"). Do X now,
   or name the exact input that blocks it.
7. **Ask only when it matters.** Reversible in-scope work proceeds without asking. Destructive
   actions, anything outward-facing (deploying, publishing, sending), and scope changes stop for
   founder approval.
8. **Docs move with the code.** Session-end doc updates (Section 1) are part of the work, not
   optional. Code and gate docs must never silently diverge (Section 10).

\---

## 15\. Skills, plugins & MCP servers (pick the right tool, safely)

🟩 **FOR THE FOUNDER** — Claude Code can be extended two ways: **skills** (pre-written expert
playbooks it can load for a specific job, like "review this design" or "fill this PDF") and **MCP
servers** (connectors that give it hands — GitHub, your database, a web browser, email). Used well,
they make builds faster and more consistent. Used carelessly, a connector is also a door into your
data — so adding one is a decision you make, not the AI.

🟦 **FOR CLAUDE CODE — tool selection rules:**

1. **Check the toolbox before improvising.** At the start of a task, check which skills and MCP
   servers are available in the session. If a purpose-built one fits (a PDF skill for PDFs, a browser
   tool for testing the running site, a GitHub connector for repo operations), use it instead of
   hand-rolling the same job with generic shell commands.
2. **Match tool to workflow step.** Typical fits: design/mockup tools at the MOCKUP step; a second
   independent model or review skill at the AUDIT step; browser automation for QA and the Definition
   of Done evidence; database connectors for schema work; GitHub tools for PRs and reviews (Layer 7).
3. **Never install or enable a new MCP server, plugin, or skill mid-build without founder approval.**
   A connector is third-party code with access to project data — that is a supply-chain decision
   (Layer 8 rule 20). When recommending one, state in plain language what it will be able to see and
   do, and where it comes from.
4. **Least-access here too.** Prefer connectors scoped to what the task needs (read-only where
   reading suffices). Production credentials never go into experimental or unvetted connectors.
5. **Name missing capabilities instead of faking them.** If a task needs a capability the session
   doesn't have (no browser tool to verify the UI, no database access to check the schema), say so
   and name what would enable it. Never simulate a result the tool would have produced.
6. **Log tool additions.** When the founder approves a new skill/server for a project, record it in
   `decisions.md` (what was added, why, what access it has).
7. **Wire the rules natively when Claude Code is the builder.** Set up the project's `.claude/`
   structure from `claude-integration/_dot-claude-template/` (see `claude-integration/
   CLAUDE-NATIVE-MAPPING.md`): the plan-auditor sub-agent runs the Section 9 audit loop, slash
   commands run the sweep/DoD checks, and hooks block unsafe operations. Verify each works before
   real building. Other builders ignore that folder — the rules are the same, only the wiring differs.
